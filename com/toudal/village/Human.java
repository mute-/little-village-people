...
public class Human {
	...
	public Human(Home pHome, Map pMap) {
		...
		do {
			ivPosX = new Double(Math.random() * ivHome.cvMapSize)
					.intValue();
			ivPosY = new Double(Math.random() * ivHome.cvMapSize)
					.intValue();
   ...
   */
	public void drawHuman() {
		...
		if (!cvDead)
			ivHome.ivOffGraphics.fillOval(new Double(ivPosX
					* ivHome.ivXFactor).intValue(), new Double(ivPosY
					* ivHome.ivYFactor).intValue(), new Double(
					1 * ivHome.ivXFactor).intValue(), new Double(
					1 * ivHome.ivYFactor).intValue());
		else {
			ivHome.ivOffGraphics.drawLine(new Double(ivPosX
					* ivHome.ivXFactor).intValue(), new Double(ivPosY
					* ivHome.ivYFactor).intValue(), new Double(
					(ivPosX + 1) * ivHome.ivXFactor).intValue(),
					new Double((ivPosY + 1) * ivHome.ivYFactor)
							.intValue());

			ivHome.ivOffGraphics.drawLine(new Double((ivPosX + 1)
					* ivHome.ivXFactor).intValue(), new Double(ivPosY
					* ivHome.ivYFactor).intValue(), new Double(ivPosX
					* ivHome.ivXFactor).intValue(), new Double(
					(ivPosY + 1) * ivHome.ivYFactor).intValue());
		}
