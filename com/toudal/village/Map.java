...
public class Map {
  ...
  public void drawBorder() {
    ...
    ivHome.ivOffGraphics.fillRect(
      0,
      0,
      ivHome.ivOffWidth - 1,
      ivHome.ivOffHeight - 1
    );

    ivHome.ivOffGraphics.setColor(Color.black);
    ivHome.ivOffGraphics.drawRect(
      0,
      0,
      ivHome.ivOffWidth - 1,
      ivHome.ivOffHeight - 1
  ...
  public void drawHouse() {
    for (int i = 0; i < ivHouseX.length; i++) {
      ivHome.ivOffGraphics.setColor(Color.black);
      ivHome.ivOffGraphics.drawRect(
        new Double(ivHouseX[i] * ivHome.ivXFactor).intValue(),
  ...
  public void drawRestArea() {
    for (int i = 0; i < ivRestAreaX.length; i++) {
      ...
      ivHome.ivOffGraphics.drawOval(
        new Double(ivRestAreaX[i] * ivHome.ivXFactor).intValue(),
  ...
  public void drawTrap() {
    for (int i = 0; i < ivTrapX.length; i++) {
      ...
      ivHome.ivOffGraphics.fillRect(
        new Double(ivTrapX[i] * ivHome.ivXFactor).intValue(),
        new Double(ivTrapY[i] * ivHome.ivYFactor).intValue(),
  ...
  public boolean isThisPosFree(int pX, int pY) {
    ...
    for (int i = 0; i < ivHouseX.length; i++) {
      ...
      if (
        ((pX >= ivHouseX[i]) && (pX < ivHouseX[i] + ivHouseWidth[i])) &&
  ...
  public boolean isThisPosRestArea(int pX, int pY) {
    ...
    for (int i = 0; i < ivRestAreaX.length; i++) {
      ...
      if (
        (
          (pX >= ivRestAreaX[i]) &&
          (pX < ivRestAreaX[i] + ivRestAreaR[i] + margin)
        ) &&
        (
          (pY >= ivRestAreaY[i]) &&
          (pY < ivRestAreaY[i] + ivRestAreaR[i] + margin)
        )
      ) {
        inside = true;
        break;
