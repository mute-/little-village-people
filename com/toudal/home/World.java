...
public class World extends TPanel implements TButtonListener {
  ...
  private World() {
    ...
    for (int j = 0; j < ivVector.size(); j++) {
      ...
      i += ivInty[j];
      if (ivListens[j]) ivButtonWeb[j].addButtonListener(this);
  ...
  public void ButtonClicked(MouseEvent mouseevent) {
    if (mouseevent.getSource() instanceof ButtonWeb) {
      ...
      for (int i = 0; i < howmany; i++) if (
        buttonweb.equals(ivButtonWeb[i]) && !ivUrls[i].equalsIgnoreCase("")
  ...
  public void ButtonEntered(MouseEvent mouseevent) {
    if (mouseevent.getSource() instanceof ButtonWeb) {
      ...
      ButtonWeb buttonweb = (ButtonWeb) mouseevent.getSource();
      for (int i = 0; i < howmany; i++) if (buttonweb.equals(ivButtonWeb[i])) {
        ivActive = i;
  ...
  public void ButtonExited(MouseEvent mouseevent) {
    if (mouseevent.getSource() instanceof ButtonWeb) {
      ...
      ButtonWeb buttonweb = (ButtonWeb) mouseevent.getSource();
      for (int i = 0; i < howmany; i++) if (
        buttonweb.equals(ivButtonWeb[i])
  ...
  public void mayUpdate() {
    if (
      ...
      cvChanged = false;
      ivTimeOver = (new Date()).getTime();
      ivButtonWeb[ivActive].doUpdate(3, ivLabels[ivActive], ivColorSpecial);
    }
  }
