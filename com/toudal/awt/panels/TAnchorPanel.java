...
public class TAnchorPanel extends TPanel {
  ...
  public void add(Component pComponent, boolean pFlag1, boolean pFlag2, boolean pFlag3, boolean pFlag4) {
    ivHashTable.put(pComponent, new TAnchorPanelInfo(pFlag1, pFlag2, pFlag3, pFlag4));
    add(pComponent);
  }
  ...
  public void placeComponents(int pX, int pY) {
    ...
    for (int k = 0; k < lComponent.length; k++) {
      TAnchorPanelInfo lTAnchorPanelInfo = (TAnchorPanelInfo) ivHashTable.get(lComponent[k]);
      if (lTAnchorPanelInfo != null) {
        Rectangle rectangle = lComponent[k].getBounds();
        if (lTAnchorPanelInfo.ivAnchorRight && lTAnchorPanelInfo.ivAnchorLeft)
          rectangle.setSize(rectangle.getSize().width + pX, rectangle.getSize().height);
        else if (lTAnchorPanelInfo.ivAnchorRight && !lTAnchorPanelInfo.ivAnchorLeft)
          rectangle.setLocation(rectangle.getLocation().x + pX, rectangle.getLocation().y);
        if (lTAnchorPanelInfo.ivAnchorTop && lTAnchorPanelInfo.ivAnchorBottom)
          rectangle.setSize(rectangle.getSize().width, rectangle.getSize().height + pY);
        else if (lTAnchorPanelInfo.ivAnchorBottom && !lTAnchorPanelInfo.ivAnchorTop)
          rectangle.setLocation(rectangle.getLocation().x, rectangle.getLocation().y + pY);
